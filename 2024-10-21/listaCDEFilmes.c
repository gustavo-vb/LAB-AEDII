...
main()
{
    ...
    f.id = 10;
    strcpy(f.nome, "Coraline");
    f.ano = 2009;
    insereOrdenado(&lista, f);

    f.id = 20;
    strcpy(f.nome, "A noiva cadaver");
    f.ano = 2006;
    insereOrdenado(&lista, f);

    f.id = 30;
    strcpy(f.nome, "Castelo animado");
    f.ano = 2004;
    insereOrdenado(&lista, f);
    ...
    if (!estaVazia(lista))
    {
        ...
        // Preparando o dado, do tipo filme, a ser removido
...
int retira(ListaCDE *l, int Dado)
{
    ...
    if ((Dado == l->inicio->dado.id) && (l->inicio == l->fim))
    {
    ...
    while ((Dado != aux->dado.id) && (aux != l->fim))
            aux = aux->prox;

    if (aux == l->fim)
    {  // se cheguei ao fim, nao encontrei o elemento
        printf("O filme com o id '%d' nao esta na lista.\n", Dado);
        return 0;
    }
    ...
    printf("\nO filme com o id '%d' foi retirado.\n", Dado);
